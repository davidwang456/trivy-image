<template>
  <v-container class="py-8">
    <v-card rounded="xl">
      <v-card-title class="text-h6">Scan by Repo</v-card-title>
      <v-card-text>
        <v-row dense>
          <v-col cols="12" md="3">
            <v-select
          ...
          <v-col cols="12" md="3">
            <v-select
              ...
              @update:model-value="onRepoChanged"
            />
          </v-col>
          <v-col cols="12" md="3">
            <v-select
...
const repoItems = computed(() => repos.value)
...
async function loadRepos() {
  ...
  try {
    ...
    repos.value = rawRepos.map((repoName) =>
...
async function loadImages() {
  ...
  error.value = undefined
  try {
    images.value = await listSwrImages(
      selectedDatasourceId.value,
      selectedNamespace.value,
      selectedRepo.value,
    )
